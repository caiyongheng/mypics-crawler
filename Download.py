...
class download():
    ...
    def __init__(self):
        ...
        self.iplist = ['182.202.220.108','49.79.194.138','218.92.220.53']  ##初始化一个list用来存放我们获取到的IP
        '''
        ...
        html = requests.get("http://ip.zdaye.com/")
        Soup = BeautifulSoup(html.text, 'lxml')
        all_tr = Soup.find('table', id = 'ipc')
        all_tr = all_tr.find_all('tr')
        for tr in all_tr:
            tr_c = tr.find_all('td')
            a_ip = tr_c[0].get_text().strip()
            self.iplist.append(a_ip)
        '''
    ...
    def get(self, url, timeout, proxy=None, num_retries=1): ## 6 给函数一个默认参数proxy为空
